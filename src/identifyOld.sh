...
#コマンドライン引数
method=old
  for numOfData in 1 2 3 4 6 7 8 9 11 12 13 14 16 17 18 19 20
  do
    for R in {1..20}
    do
      for T in 6
      do
        ...
        for n in {1..100}
        do
          java identifyMacAddress/identify/identifyStay data/capture/convert/$numOfData/$n,convertData.csv $R $T
           > data/result/multi/move/$numOfTime/$method/$numOfData/$R,$T,$n.txt
        done
        if [ "$R" -eq "1" -a "$I" -eq "1" ]
        then
