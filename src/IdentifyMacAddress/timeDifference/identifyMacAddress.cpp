...
inline double getR(Address address, Address nextAddress)
{
  ...
  for (int i = 0; i < nextAddress.getFPackets().size(); i++)
  {
    for (int j = 0; j < address.getRegression().size(); j++)
    {
      ...
      if (address.getRegression()[j].getTime() == nextAddress.getFPackets()[i].getTime()&&address.getRegression()[j].getRssi()>=-100)
      {
...
std::vector<Address> normalize(Address address, int R, int T)
{
  ...
  {
    address.getNextAddressList()[i].setNormalizedT(std::pow(address.getNextAddressList()[i].getFTime() - address.getLTime());
  ...
  for(int i=0;i<addressList.size();i++){
    for(int j=0;j<addressList.size();j++){
      ...
      for(int k=0;k<addressList[i].getNextAddressList().size();k++){
         ...
         double sub =addressList[i].getNextAddressList()[k].getFTime()-addressList[i].getLTime();
         if(addressList[j].getAddress()==addressList[i].getNextAddressList()[k].getAddress()&&sub>=0)
            output=sub;
      }
      writing_file << output;
